...
public class WooriBankService {
	...
	public void menu(Scanner inputInt, Scanner inputStr, AccountDTO[] wooriBank) {
		...
		while (subRun) {
			System.out.println("--------------------------------------");
			System.out.println("우리은행계좌관리 메서드입니다.");
			...
			case "1":
				...
				createAccount(inputStr, inputStr, wooriBank);
				break;

			case "2":
				System.out.println("===계좌목록메서드로 진입합니다===");
				accountLists(inputStr, inputInt, wooriBank);
				break;

			case "3":
				System.out.println("===예금메서드로 진입합니다===");
				deposit(inputStr, inputInt, wooriBank);
				break;

			case "4":
				System.out.println("===출금메서드로 진입합니다===");
				withDraw(inputStr, inputInt, wooriBank);
				break;

			case "5":
				System.out.println("===이체메서드로 진입합니다===");
				System.out.println("1. 하나은행 내 송금");
				System.out.println("2. 타 은행 간 송금");
				System.out.println(">>>");
				String tselect = inputStr.next();

				if (tselect.equals("1")) {
					transferSameBank(inputStr, inputInt, wooriBank);
				} else if (tselect.equals("2")) {
					transfterOtherBank(inputStr, inputInt, senderBank, receiver );
	...
	private void transferSameBank(Scanner inputStr, Scanner inputInt, AccountDTO[] wooriBank) {

		System.out.println("우리은행 간 송금 페이지입니다");
		...
		AccountDTO sender = findAccount(sendAno, wooriBank); 
		AccountDTO receiver = findAccount(receiveAno, wooriBank);
	...
	private void withDraw(Scanner inputStr, Scanner inputInt, AccountDTO[] wooriBank) {
		...
		int money = inputInt.nextInt();
		AccountDTO withdrawalAccount = findAccount(ano, wooriBank);
	...
	private void accountLists(Scanner inputStr, Scanner inputInt, AccountDTO[] wooriBank) {
		// 하나은행에 계정을 모두 본다 (은행원용)
		System.out.println("------우리은행계좌리스트------");
		for (int i = 0; i < wooriBank.length; i++) {
	...
	private void createAccount(Scanner inputStr, Scanner inputInt, AccountDTO[] wooriBank) {
		...
		myAccount.setBankname("우리"); // 빈 객체에 데이터 입력완료

		// 하나 은행 배열에 객체를 삽입 (null인지 확인하고 넣음)
		for (int i = 0; i < wooriBank.length; i++) {
			if (wooriBank[i] == null) {
				wooriBank[i] = myAccount; // 배열에 입력한 객체 연결
				System.out.println("계좌 입력 완료 테스트 : " + wooriBank[i]);
				break; // 1개만 입력하기 위해
