...
public class HanaBankService { // 서비스 계층은 CRUD 메서드 위주로 생성한다
	...
	public void menu(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank) {
		...
		while (subRun) {
			...
			case "5":
				...
				if (tselect.equals("1")) {
					transferSameBank(inputStr, inputInt, hanaBank);
				} else if (tselect.equals("2")) {
					transfterOtherBank(inputStr, inputInt, hanaBank, wooriBank, nhBank);
				}
				break;
	...
	private void transferSameBank(Scanner inputStr, Scanner inputInt, AccountDTO[] hanaBank) {

		System.out.println("하나은행 간 송금 페이지입니다");
		...
		AccountDTO sender = findAccount(sendAno, hanaBank); 
		AccountDTO receiver = findAccount(receiveAno, hanaBank);
	...
	private void withDraw(Scanner inputStr, Scanner inputInt, AccountDTO[] hanaBank) {
		...
		int money = inputInt.nextInt();
		AccountDTO withdrawalAccount = findAccount(ano, hanaBank);
	...
	private void createAccount(Scanner inputStr, Scanner inputInt, AccountDTO[] hanaBank) {
		...
		myAccount.setBankname("하나"); // 빈 객체에 데이터 입력완료
